...
class Virustotal:
    ...
    def upload_file(self, filename, force=False):
        if self.check_file_exist(filename) and force:
            return None
    ...
    # def load_html(self,url) -> bs4.BeautifulSoup:
       
    #     return bs4.BeautifulSoup(self.session.get(url,headers=self.upload_headers, cookies={"VT_PREFERRED_LANGUAGE": "en"}).text,"lxml")
